...
double stiffness = 1000; // N/m
double manipulandumGravityCompensation = 0.0; //0.2; // N
...
// a frequency counter to measure the simulation bullet rate
cFrequencyCounter freqCounterDynamics;
...
int m_nextcolor = 0;
cColorf colors[] = {                          hex2rgb(0x9acd32),
                                              hex2rgb(0xfa8072),
                                              hex2rgb(0xffefd5),
                                              hex2rgb(0xf8f8ff),
                                              hex2rgb(0xfffaf0),
...
int main(int argc, char* argv[])
{
    ...
    bulletWorld->m_backgroundColor.setWhiteIvory();
...
/*
    ...
    cDirectionalLight *dlight = new cDirectionalLight(bulletWorld);
    // insert light source inside world
    bulletWorld->addChild(dlight);
    // enable light source
    dlight->setEnabled(true);
    // define direction of light beam
    dlight->setDir(-1.0, -0.0, -1.0);
   */
    ...
    numDevices = cMin(handler->getNumDevices(),(unsigned int)(2));
    ...
    cVector3d devicePos;
    hapticDevice[0]->getPosition(devicePos);
    std::cout << "Intial pos: " << devicePos << "\n";
    ...
    for(int i=0;i<numDevices;++i){
        ...
        tool[i]->getHapticPoint(0)->setShow(false,false); //was false false
#endif
    }
...
/*
        if(i==5){
            ...
            std::string filename = "polhem_slide2.jpg";//"bg.png";
            walls[i]->m_texture->loadFromFile(filename);
...
void updateGraphics(void)
{
    ...
    labelRates->setText(cStr(freqCounterGraphics.getFrequency(), 0) + " Hz / " +
        ...
        cStr(freqCounterDynamics.getFrequency(), 0) + " Hz");
...
void updateHaptics(void)
{
    ...
    while(simulationRunning)
    {
        ...
        for(int i=0;i<numDevices;++i){
            ...
            //std::cout << tool[i]->getDeviceGlobalPos() <<"\n";
            //cVector3d devicePos;
            //hapticDevice[0]->getPosition(devicePos);
            //std::cout << "Intial pos: " << devicePos << "\n";
            ...
            // Show (for debug)
            deviceSphere[i]->setLocalPos(cVector3d(1000,0,0));//tool[i]->getDeviceGlobalPos());
